...
date: 2026-08-05
category: Django
source_url: https://pythondjango.cn/django/rest-framework/9-filter-ordering/
author: 大江狗
tags:
  - Django
  - status/done
...
# 过滤与排序
## 目录
1. [前情回顾](#%E5%89%8D%E6%83%85%E5%9B%9E%E9%A1%BE)
2. [方法一: 重写get\_queryset方法](#%E6%96%B9%E6%B3%95%E4%B8%80-%E9%87%8D%E5%86%99get_queryset%E6%96%B9%E6%B3%95)
3. [方法二: 使用django-filter](#%E6%96%B9%E6%B3%95%E4%BA%8C-%E4%BD%BF%E7%94%A8django-filter)
   1. [安装django-filter](#%E5%AE%89%E8%A3%85django-filter)
   2. [自定义FilterSet类](#%E8%87%AA%E5%AE%9A%E4%B9%89filterset%E7%B1%BB)
4. [方法三: 使用SearchFilter类](#%E6%96%B9%E6%B3%95%E4%B8%89-%E4%BD%BF%E7%94%A8searchfilter%E7%B1%BB)
   1. [自定义SearchFilter类](#%E8%87%AA%E5%AE%9A%E4%B9%89searchfilter%E7%B1%BB)
5. [排序OrderingFilter类](#%E6%8E%92%E5%BA%8Forderingfilter%E7%B1%BB)
...
![img](../assets/django/rest-framework/9-filter-ordering.assets/6.png)
## 方法一: 重写get\_queryset方法
...
![img](../assets/django/rest-framework/9-filter-ordering.assets/2.png)
...
```
当一个模型需要过滤的字段很多且不确定时(比如文章状态、正文等等), 重写get\_queryset方法将变得非常麻烦，更好的方式是借助django-filter这个第三方库实现过滤。
## 方法二: 使用django-filter
...
```
把`django_filters`添加到INSTALLED\_APPS中去。
```
INSTALLED_APPS = [
    ...,
    django_filters,
...
![img](../assets/django/rest-framework/9-filter-ordering.assets/4.png)
...
![img](../assets/django/rest-framework/9-filter-ordering.assets/5.png)
...
```
默认情况下，SearchFilter类搜索将使用不区分大小写的部分匹配(icontains)。你可以在search\_fields中添加各种字符来指定匹配方法。
* ’^’开始 - 搜索。
* ’=’完全匹配。
* ’@’全文搜索。
* ’$’正则表达式搜索。
例如：search\_fields = (‘=title’, )精确匹配title。
### 自定义SearchFilter类
默认SearchFilter类仅支持`?search=xxx`这个传递参数，你可以通过设置`SEARCH_PARAM`覆盖。另外你还可以重写`get_search_fileds`方法改变它的行为。下例中，当你按照/?search=keyword&title\_only=True提交请求时，它将只针对title进行查询。
```
from rest_framework import filters
...
```
发送请求时只需要在参数上加上?ordering=create\_date或者?ordering=-create\_date即可实现对结果按文章创建时间正序和逆序进行排序。
点击DRF界面上的Filters按钮，你还会看到搜索和排序的选项。
![img](../assets/django/rest-framework/9-filter-ordering.assets/3.png)
