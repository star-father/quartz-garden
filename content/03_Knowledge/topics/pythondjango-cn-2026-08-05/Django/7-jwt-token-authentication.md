...
date: 2026-08-05
category: Django
source_url: https://pythondjango.cn/django/rest-framework/7-jwt-token-authentication/
author: 大江狗
tags:
  - Django
  - status/done
...
# JWT认证
## 目录
1. [什么是Json Web Token?](#%E4%BB%80%E4%B9%88%E6%98%AFjson-web-token)
2. [DRF中如何使用JWT认证](#drf%E4%B8%AD%E5%A6%82%E4%BD%95%E4%BD%BF%E7%94%A8jwt%E8%AE%A4%E8%AF%81)
   1. [安装](#%E5%AE%89%E8%A3%85)
   2. [测试](#%E6%B5%8B%E8%AF%95)
3. [Simple JWT的默认设置](#simple-jwt%E7%9A%84%E9%BB%98%E8%AE%A4%E8%AE%BE%E7%BD%AE)
4. [自定义令牌(token)](#%E8%87%AA%E5%AE%9A%E4%B9%89%E4%BB%A4%E7%89%8Ctoken)
5. [自定义认证后台(Backend)](#%E8%87%AA%E5%AE%9A%E4%B9%89%E8%AE%A4%E8%AF%81%E5%90%8E%E5%8F%B0backend)
...
![img](../assets/django/rest-framework/7-jwt-token-authentication.assets/6.png)
...
![img](../assets/django/rest-framework/7-jwt-token-authentication.assets/8.png)
...
![img](../assets/django/rest-framework/7-jwt-token-authentication.assets/7.png)
假如你有一个受保护的视图(比如这里的/image/)，权限(permission\_classes)是`IsAuthenticated`，只有验证用户才可访问。访问这个保护视图时你只需要在请求头的Authorization选项里输入你刚才获取的access token即可，如下所示：
![img](../assets/django/rest-framework/7-jwt-token-authentication.assets/9.png)
不过这个access token默认只有5分钟有效。5分钟过后，当你再次访问保护视图时，你将得到如下token已失效或过期的提示：
![img](../assets/django/rest-framework/7-jwt-token-authentication.assets/5.png)
...
![img](../assets/django/rest-framework/7-jwt-token-authentication.assets/2.png)
...
```
## 自定义令牌(token)
如果你对Simple JWT返回的access token进行解码，你会发现这个token的payload数据部分包括token类型，token失效时间，jti(一个类似随机字符串）和user\_id。如果你希望在payload部分提供更多信息，比如用户的username，这时你就要自定义令牌(token)了。
![img](../assets/django/rest-framework/7-jwt-token-authentication.assets/8.png)
...
![img](../assets/django/rest-framework/7-jwt-token-authentication.assets/3.png)
对重新获取的access token进行解码，你将看到payload部分多了username的内容，是不是很酷? 在实际API开发过程中，通过Json Web Token传递更多数据非常有用。
![img](../assets/django/rest-framework/7-jwt-token-authentication.assets/4.png)
## 自定义认证后台(Backend)
...
class MyCustomBackend(ModelBackend):
    def authenticate(self, request, username=None, password=None, **kwargs):
        try:
            ...
            return None
```
**其次**，修改`settings.py`, 把你自定义的验证方式添加到AUTHENTICATION\_BACKENDS里去。
```
