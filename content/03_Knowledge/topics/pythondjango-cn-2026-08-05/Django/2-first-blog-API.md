---
type: topic
title: "以博客为例使用DRF的序列化器和基于函数的视图开发API"
date: 2026-08-05
category: Django
source_url: https://pythondjango.cn/django/rest-framework/2-first-blog-API/
author: 大江狗
tags:
  - Django
  - status/done
...
1. [准备工作](#%E5%87%86%E5%A4%87%E5%B7%A5%E4%BD%9C)
   1. [创建模型 (models)](#%E5%88%9B%E5%BB%BA%E6%A8%A1%E5%9E%8B-models)
   2. [配置Django后台(admin)](#%E9%85%8D%E7%BD%AEdjango%E5%90%8E%E5%8F%B0admin)
2. [自定义序列化器(serializers)](#%E8%87%AA%E5%AE%9A%E4%B9%89%E5%BA%8F%E5%88%97%E5%8C%96%E5%99%A8serializers)
   1. [使用Serializer类](#%E4%BD%BF%E7%94%A8serializer%E7%B1%BB)
   2. [使用ModelSerializer类](#%E4%BD%BF%E7%94%A8modelserializer%E7%B1%BB)
3. [编写API视图(views.py)](#%E7%BC%96%E5%86%99api%E8%A7%86%E5%9B%BEviewspy)
4. [给URLs添加可选的格式后缀](#%E7%BB%99urls%E6%B7%BB%E5%8A%A0%E5%8F%AF%E9%80%89%E7%9A%84%E6%A0%BC%E5%BC%8F%E5%90%8E%E7%BC%80)
5. [API测试](#api%E6%B5%8B%E8%AF%95)
   1. [GET请求](#get%E8%AF%B7%E6%B1%82)
   2. [POST请求](#post%E8%AF%B7%E6%B1%82)
   3. [PUT请求](#put%E8%AF%B7%E6%B1%82)
   4. [其它请求](#%E5%85%B6%E5%AE%83%E8%AF%B7%E6%B1%82)
...
本例中ID和create\_date都是由模型自动生成，每个article的author我们也希望在视图中与request.user绑定，而不是由用户通过POST或PUT自行修改，所以这些字段都是read-only。相反title，body和status是用户可以添加或修改的字段，所以未设成read-only。
### 使用ModelSerializer类
...
![img](../assets/django/rest-framework/2-first-blog-API.assets/6.png)
注意：DRF默认是以可浏览的api形式展示返回响应结果的(`articles.api`)，如果你只需要返回最简单的json格式的数据，只需要在访问地址后面加上`.json`后缀即可(articles.json)，如下所示：
![img](../assets/django/rest-framework/2-first-blog-API.assets/3.png)
```
[GET] http://127.0.0.1:8000/v1/articles/3
```
发送get请求到/v1/articles/3，获取第3篇文章详情。 ![img](../assets/django/rest-framework/2-first-blog-API.assets/1.png)
### POST请求
```
[POST] http://127.0.0.1:8000/v1/articles
```
发送POST请求到/v1/articles。在GET页面下方下拉框选择json格式数据或者表单，新增文章数据，点击POST提交后即可看到新的文章已经添加。
![img](../assets/django/rest-framework/2-first-blog-API.assets/4.png)
### PUT请求
```
...
![img](../assets/django/rest-framework/2-first-blog-API.assets/5.png)
### 其它请求
