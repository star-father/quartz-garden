...
date: 2026-08-05
category: Django
source_url: https://pythondjango.cn/django/rest-framework/4-DRF-serializer-advanced/
author: 大江狗
tags:
  - Django
  - status/done
...
2. [改变序列化器的输出](#%E6%94%B9%E5%8F%98%E5%BA%8F%E5%88%97%E5%8C%96%E5%99%A8%E7%9A%84%E8%BE%93%E5%87%BA)
   1. [指定source来源](#%E6%8C%87%E5%AE%9Asource%E6%9D%A5%E6%BA%90)
   2. [使用SerializerMethodField方法](#%E4%BD%BF%E7%94%A8serializermethodfield%E6%96%B9%E6%B3%95)
   3. [使用 to\_representation方法](#%E4%BD%BF%E7%94%A8-to_representation%E6%96%B9%E6%B3%95)
   4. [使用嵌套序列化器](#%E4%BD%BF%E7%94%A8%E5%B5%8C%E5%A5%97%E5%BA%8F%E5%88%97%E5%8C%96%E5%99%A8)
   5. [设置关联模型深度](#%E8%AE%BE%E7%BD%AE%E5%85%B3%E8%81%94%E6%A8%A1%E5%9E%8B%E6%B7%B1%E5%BA%A6)
   6. [序列化日期和时间](#%E5%BA%8F%E5%88%97%E5%8C%96%E6%97%A5%E6%9C%9F%E5%92%8C%E6%97%B6%E9%97%B4)
3. [关系序列化](#%E5%85%B3%E7%B3%BB%E5%BA%8F%E5%88%97%E5%8C%96)
   1. [PrimaryKeyRelatedField](#primarykeyrelatedfield)
   2. [StringRelatedField](#stringrelatedfield)
   3. [HyperlinkedRelatedField](#hyperlinkedrelatedfield)
4. [数据验证 (Validation)](#%E6%95%B0%E6%8D%AE%E9%AA%8C%E8%AF%81-validation)
   1. [字段级别验证 (Field-level validation)](#%E5%AD%97%E6%AE%B5%E7%BA%A7%E5%88%AB%E9%AA%8C%E8%AF%81-field-level-validation)
   2. [对象级别验证 (Object-level validation)](#%E5%AF%B9%E8%B1%A1%E7%BA%A7%E5%88%AB%E9%AA%8C%E8%AF%81-object-level-validation)
   3. [验证器 (Validators)](#%E9%AA%8C%E8%AF%81%E5%99%A8-validators)
5. [重写序列化器的create和update方法](#%E9%87%8D%E5%86%99%E5%BA%8F%E5%88%97%E5%8C%96%E5%99%A8%E7%9A%84create%E5%92%8Cupdate%E6%96%B9%E6%B3%95)
6. [动态加载序列化器类](#%E5%8A%A8%E6%80%81%E5%8A%A0%E8%BD%BD%E5%BA%8F%E5%88%97%E5%8C%96%E5%99%A8%E7%B1%BB)
...
```
![img](../assets/django/rest-framework/4-DRF-serializer-advanced.assets/6.png)
...
class ArticleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ('id', 'author', 'create_date')
```
这时你应该可以看到想要的输出数据格式了，是不是很赞? ![img](../assets/django/rest-framework/4-DRF-serializer-advanced.assets/3.png)
...
上面例子中文章状态status都是以Published或Draft英文字符串表示的，但是如果你想在输出的json格式数据中新增`cn_status`字段，显示中文发表状态。但cn\_status本身并不是Article模型中存在的字段，这时你应该怎么做呢？答案是使用`SerializerMethodField`，它可用于将任何类型的数据添加到对象的序列化表示中, 非常有用。
...
```
这时的输出数据是不是让人赏心悦目？
![img](../assets/django/rest-framework/4-DRF-serializer-advanced.assets/4-1.jpg)
不过需要注意的是`SerializerMethodField`通常用于显示模型中原本不存在的字段，类似可读字段，你不能通过反序列化对其直接进行修改。
### 使用 to\_representation方法
...
```
展示效果如下所示：
![img](../assets/django/rest-framework/4-DRF-serializer-advanced.assets/5.png)
...
![img](../assets/django/rest-framework/4-DRF-serializer-advanced.assets/8.png)
...
![image-20210514212054568](../assets/django/rest-framework/4-DRF-serializer-advanced.assets/image-20210514212054568.png)
...
![image-20210514212400732](../assets/django/rest-framework/4-DRF-serializer-advanced.assets/image-20210514212400732.png)
...
```
返回结果如下所示：
![image-20210514213211344](../assets/django/rest-framework/4-DRF-serializer-advanced.assets/image-20210514213211344.png)
## 数据验证 (Validation)
...
```
## 小结
* 改变序列化输出数据的格式可以通过指定字段的source来源，使用SerializerMethodField和to\_representation方法以及使用嵌套序列化器。
