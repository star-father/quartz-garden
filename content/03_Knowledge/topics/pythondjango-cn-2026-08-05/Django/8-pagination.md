...
date: 2026-08-05
category: Django
source_url: https://pythondjango.cn/django/rest-framework/8-pagination/
author: 大江狗
tags:
  - Django
  - status/done
...
2. [DRF提供的分页类](#drf%E6%8F%90%E4%BE%9B%E7%9A%84%E5%88%86%E9%A1%B5%E7%B1%BB)
   1. [PageNumberPagination类](#pagenumberpagination%E7%B1%BB)
   2. [LimitOffsetPagination类](#limitoffsetpagination%E7%B1%BB)
   3. [CursorPagination类](#cursorpagination%E7%B1%BB)
3. [函数类视图中使用分页类](#%E5%87%BD%E6%95%B0%E7%B1%BB%E8%A7%86%E5%9B%BE%E4%B8%AD%E4%BD%BF%E7%94%A8%E5%88%86%E9%A1%B5%E7%B1%BB)
...
* **PageNumberPagination**：普通分页器。支持用户按?page=3&size=10这种更灵活的方式进行查询，这样用户不仅可以选择页码，还可以选择每页展示数据的数量。通常还需要设置max\_page\_size这个参数限制每页展示数据的最大数量，以防止用户进行恶意查询(比如size=10000), 这样一页展示1万条数据将使分页变得没有意义。
* **LimitOffsetPagination**：偏移分页器。支持用户按?limit=20&offset=100这种方式进行查询。offset是查询数据的起始点，limit是每页展示数据的最大条数，类似于page\_size。当你使用这个类时，你通常还需要设置max\_limit这个参数来限制展示给用户数据的最大数量。
...
![img](../assets/django/rest-framework/8-pagination.assets/7.png)
...
class ArticleViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        serializer.save(author=self.request.user)
```
展示效果如下所示： ![img](../assets/django/rest-framework/8-pagination.assets/3.png)
...
```
新的展示效果如下所示：
![img](../assets/django/rest-framework/8-pagination.assets/2.png)
...
![img](../assets/django/rest-framework/8-pagination.assets/6.png)
...
```
展示效果如下所示：
![img](../assets/django/rest-framework/8-pagination.assets/4.png)
...
```
响应效果如下所示，你将得到previous和next分页链接。页码都加密了, 链接里不再显示页码号码。默认每页展示3条记录, 如果使用?page\_size=2进行查询，每页你将得到两条记录。
![img](../assets/django/rest-framework/8-pagination.assets/5.png)
