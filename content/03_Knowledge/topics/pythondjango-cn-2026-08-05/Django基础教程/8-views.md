...
date: 2026-08-05
category: Django基础教程
source_url: https://pythondjango.cn/django/basics/8-views/
author: 大江狗
tags:
  - Django基础教程
  - status/done
...
# Django函数视图及通用类视图
## 目录
1. [什么是视图(View)及其工作原理](#%E4%BB%80%E4%B9%88%E6%98%AF%E8%A7%86%E5%9B%BEview%E5%8F%8A%E5%85%B6%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86)
2. [接近现实的函数视图](#%E6%8E%A5%E8%BF%91%E7%8E%B0%E5%AE%9E%E7%9A%84%E5%87%BD%E6%95%B0%E8%A7%86%E5%9B%BE)
3. [更复杂的案例: 视图处理用户提交的数据](#%E6%9B%B4%E5%A4%8D%E6%9D%82%E7%9A%84%E6%A1%88%E4%BE%8B-%E8%A7%86%E5%9B%BE%E5%A4%84%E7%90%86%E7%94%A8%E6%88%B7%E6%8F%90%E4%BA%A4%E7%9A%84%E6%95%B0%E6%8D%AE)
4. [基于函数的视图和基于类的视图](#%E5%9F%BA%E4%BA%8E%E5%87%BD%E6%95%B0%E7%9A%84%E8%A7%86%E5%9B%BE%E5%92%8C%E5%9F%BA%E4%BA%8E%E7%B1%BB%E7%9A%84%E8%A7%86%E5%9B%BE)
5. [Django通用类视图](#django%E9%80%9A%E7%94%A8%E7%B1%BB%E8%A7%86%E5%9B%BE)
   1. [ListView](#listview)
   2. [DetailView](#detailview)
   3. [CreateView](#createview)
   4. [UpdateView](#updateview)
   5. [FormView](#formview)
...
class ArticleDetailView(DetailView):
    model = Article
```
`DetailView`默认的模板是`app/model_name_detail.html`,默认的内容对象名字`context_object_name`是model\_name。本例中默认模板是`blog/article_detail.html`, 默认对象名字是`article`, 在模板里可通过 ``获取文章标题。
...
class ArticleCreateView(CreateView):
    ...
    fields = ['title', 'body',]
```
CreateView默认的模板是`model_name_form.html,` 即`article_form.html`。这里CreateView还会根据`fields`自动生成表单字段。默认的context\_object\_name是`form`。模板代码如下图所示:
```
...
```
form\_valid方法一个常见用途就是就是将创建对象的用户与model里的user结合（需要用户先登录再提交)。见下面例子。
```
...
* 虽然form\_valid方法不是必需，但很有用。当用户提交的数据是有效的时候，你可以通过定义此方法做些别的事情，比如发送邮件，存取额外的数据。
```
from django.views.generic.edit import UpdateView
from .models import Article
...
```
### DeleteView
DeleteView一般用来删除某个具体对象。它要求用户点击确认后再删除一个对象。使用这个通用视图，你需要定义模型的名称model和成功删除对象后的返回的URL。默认模板是`myapp/model_confirm_delete.html`。默认内容对象名字是model\_name。本例中为article。
