...
date: 2026-08-05
category: Django基础教程
source_url: https://pythondjango.cn/django/basics/17-settings/
author: 大江狗
tags:
  - Django基础教程
  - status/done
...
# Django配置文件
## 目录
1. [BASE\_DIR](#base_dir)
2. [DEBUG](#debug)
3. [ALLOWED\_HOSTS](#allowed_hosts)
4. [SECRET\_KEY](#secret_key)
   1. [方法一: 从环境变量中读取SECRET\_KEY](#%E6%96%B9%E6%B3%95%E4%B8%80-%E4%BB%8E%E7%8E%AF%E5%A2%83%E5%8F%98%E9%87%8F%E4%B8%AD%E8%AF%BB%E5%8F%96secret_key)
   2. [方法二: 从服务器上Django项目文件价外的某个文件读取](#%E6%96%B9%E6%B3%95%E4%BA%8C-%E4%BB%8E%E6%9C%8D%E5%8A%A1%E5%99%A8%E4%B8%8Adjango%E9%A1%B9%E7%9B%AE%E6%96%87%E4%BB%B6%E4%BB%B7%E5%A4%96%E7%9A%84%E6%9F%90%E4%B8%AA%E6%96%87%E4%BB%B6%E8%AF%BB%E5%8F%96)
5. [INSTALLED\_APPS](#installed_apps)
6. [AUTH\_USER\_MODEL](#auth_user_model)
7. [STATIC\_ROOT和STATIC\_URL](#static_root%E5%92%8Cstatic_url)
8. [STATICFILES\_DIRS](#staticfiles_dirs)
9. [MEDIA\_ROOT和MEDIA\_URL](#media_root%E5%92%8Cmedia_url)
10. [国际化(语言与时间)](#%E5%9B%BD%E9%99%85%E5%8C%96%E8%AF%AD%E8%A8%80%E4%B8%8E%E6%97%B6%E9%97%B4)
11. [邮箱服务配置](#%E9%82%AE%E7%AE%B1%E6%9C%8D%E5%8A%A1%E9%85%8D%E7%BD%AE)
12. [模板设置](#%E6%A8%A1%E6%9D%BF%E8%AE%BE%E7%BD%AE)
13. [中间件设置](#%E4%B8%AD%E9%97%B4%E4%BB%B6%E8%AE%BE%E7%BD%AE)
14. [数据库设置](#%E6%95%B0%E6%8D%AE%E5%BA%93%E8%AE%BE%E7%BD%AE)
15. [缓存设置](#%E7%BC%93%E5%AD%98%E8%AE%BE%E7%BD%AE)
16. [Session相关设置](#session%E7%9B%B8%E5%85%B3%E8%AE%BE%E7%BD%AE)
17. [小结](#%E5%B0%8F%E7%BB%93)
---
Django设置文件settings.py包含的选项非常多，但好消息是大部分不需要我们手动去设置。本章就来看下一些常用设置选项及它们背后的含义。
当我们使用`jango-admin.py startproject xxx`命令创建一个Django项目时，你会发现生成的`settings.py`已经包含了部分基本的默认设定，我们只需要修改和添加我们需要使用的设定就好了。一个项目完整的全局默认设置在`django/conf/global_settings.py`文件中。Django在编译时，会先载入global\_settings.py中的全局默认配置值，然后加载用户指定的`settings.py`，重写部分全局默认设置。
## BASE\_DIR
...
## ALLOWED\_HOSTS
默认值为空`[]`。设置ALLOWED\_HOSTS是为了限定用户请求中的host值，以防止黑客构造包来进行头部攻击。该选项正确设置方式如下:
* DEBUG=True: ALLOWED\_HOSTS可以为空，也可设置为[‘127.0.0.01’, ‘localhost’]
* DEBUG=False: ALLOWED\_HOSTS=[‘46.124.78.xx’, ‘www.bat.com’，’127.0.0.1’]
...
## SECRET\_KEY
SECRET\_KEY是Django根据自己算法生成的一大串随机数，本质是个加密盐，用于防止CSRF（Cross-site request forgery）跨站请求伪造攻击。当部署Django项目到生产环境中时，Django文档建议不直接在`settings.py`里输入字符串，而是采取下面几种方法读取SECRET\_KEY。
### 方法一: 从环境变量中读取SECRET\_KEY
```
import os
...
## STATIC\_ROOT和STATIC\_URL
这两个选项是关于静态文件(如CSS, JS,和图片)的最重要的设置，一般设置如下。STATIC\_URL是静态文件URL，设置后可以通过使用`{% static 'assets/imges/xxx.jpg' %}`方式直接访问/static/文件夹里的静态文件。如果你设置了STATIC\_ROOT, 当你运行`python manage.py collectstatic`命令的时候，Django会将各app下所有名为static的文件夹及其子目录复制收集到STATIC\_ROOT。把静态文件集中一起的目的是为了更方便地通过Apache或Nginx部署。
```
STATIC_URL = '/static/'
