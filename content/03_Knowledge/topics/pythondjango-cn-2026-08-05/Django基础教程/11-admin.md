...
date: 2026-08-05
category: Django基础教程
source_url: https://pythondjango.cn/django/basics/11-admin/
author: 大江狗
tags:
  - Django基础教程
  - status/done
...
# Django管理后台(Admin)入门
## 目录
1. [为什么要使用Django Admin](#%E4%B8%BA%E4%BB%80%E4%B9%88%E8%A6%81%E4%BD%BF%E7%94%A8django-admin)
2. [如何使用Django Admin](#%E5%A6%82%E4%BD%95%E4%BD%BF%E7%94%A8django-admin)
   1. [创建超级用户superuser](#%E5%88%9B%E5%BB%BA%E8%B6%85%E7%BA%A7%E7%94%A8%E6%88%B7superuser)
   2. [注册模型](#%E6%B3%A8%E5%86%8C%E6%A8%A1%E5%9E%8B)
   3. [自定义数据表显示选项](#%E8%87%AA%E5%AE%9A%E4%B9%89%E6%95%B0%E6%8D%AE%E8%A1%A8%E6%98%BE%E7%A4%BA%E9%80%89%E9%A1%B9)
   4. [自定义list\_display显示字段](#%E8%87%AA%E5%AE%9A%E4%B9%89list_display%E6%98%BE%E7%A4%BA%E5%AD%97%E6%AE%B5)
   5. [优化ForeignKey或多对多字段的选择](#%E4%BC%98%E5%8C%96foreignkey%E6%88%96%E5%A4%9A%E5%AF%B9%E5%A4%9A%E5%AD%97%E6%AE%B5%E7%9A%84%E9%80%89%E6%8B%A9)
   6. [使用Inline表单](#%E4%BD%BF%E7%94%A8inline%E8%A1%A8%E5%8D%95)
3. [小结](#%E5%B0%8F%E7%BB%93)
---
...
![image-20210321131935321](../assets/django/basics/11-admin.assets/image-20210321131935321.png)
### 注册模型
...
![image-20210321132353304](../assets/django/basics/11-admin.assets/image-20210321132353304.png)
### 自定义数据表显示选项
...
![img](../assets/django/basics/11-admin.assets/2.png)
### 自定义list\_display显示字段
...
```
新的展示效果如下所示：
![img](../assets/django/basics/11-admin.assets/11.png)
...
![img](../assets/django/basics/11-admin.assets/8.png)
*再修改admin.py, 使用raw\_id\_fields*
```
# Register your models here.
class ArticleAdmin(admin.ModelAdmin):
    list_display = ('title', 'author', 'status', 'create_date', 'custom_date', )
...
![img](../assets/django/basics/11-admin.assets/1.png)
本例模型中不涉及多对多字段，但在实际应用中django admin对于多对多字段使用多选下拉菜单，也非常不方便，建议选择使用`filter_horizontal`或`filter_vertical`设置，其展示效果为双向穿梭选择器，如下所示：
![img](../assets/django/basics/11-admin.assets/9.png)
### 使用Inline表单
本例中author和article是单对多的关系，前面案例中文章都是逐一添加的。如果我们希望在创建或编辑用户的时候，就一次性添加多篇文章怎么操作呢? 答案是使用inline表单。
我们首先用Article模型创建两个Inline类，一个用于展示和编辑已创建的文章列表，一个用于一次性添加多篇文章。由于Article模型可能有多个ForeignKey(比如category或author), 我们这里必需通过fk\_name选项指定在那个ForeinKey的模型里使用创建的inlines类。
```
...
![img](../assets/django/basics/11-admin.assets/10.png)
## 小结
