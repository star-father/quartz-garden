...
date: 2026-08-05
category: Django进阶教程
source_url: https://pythondjango.cn/django/advanced/9-middleware/
author: 大江狗
tags:
  - Django进阶教程
  - status/done
...
# Django中间件原理及示例
## 目录
1. [什么是中间件及其应用场景](#%E4%BB%80%E4%B9%88%E6%98%AF%E4%B8%AD%E9%97%B4%E4%BB%B6%E5%8F%8A%E5%85%B6%E5%BA%94%E7%94%A8%E5%9C%BA%E6%99%AF)
2. [Django自带中间件](#django%E8%87%AA%E5%B8%A6%E4%B8%AD%E9%97%B4%E4%BB%B6)
3. [Django的中间件执行顺序](#django%E7%9A%84%E4%B8%AD%E9%97%B4%E4%BB%B6%E6%89%A7%E8%A1%8C%E9%A1%BA%E5%BA%8F)
4. [自定义中间件](#%E8%87%AA%E5%AE%9A%E4%B9%89%E4%B8%AD%E9%97%B4%E4%BB%B6)
   1. [函数](#%E5%87%BD%E6%95%B0)
   2. [使用类](#%E4%BD%BF%E7%94%A8%E7%B1%BB)
5. [其它中间件钩子函数](#%E5%85%B6%E5%AE%83%E4%B8%AD%E9%97%B4%E4%BB%B6%E9%92%A9%E5%AD%90%E5%87%BD%E6%95%B0)
   1. [process\_view(request, view\_func, view\_args, view\_kwargs)](#process_viewrequest-view_func-view_args-view_kwargs)
   2. [process\_exception(self, request, exception)](#process_exceptionself-request-exception)
   3. [process\_template\_response(self, request, response)](#process_template_responseself-request-response)
   4. [如何使用这3个钩子函数？](#%E5%A6%82%E4%BD%95%E4%BD%BF%E7%94%A8%E8%BF%993%E4%B8%AA%E9%92%A9%E5%AD%90%E5%87%BD%E6%95%B0)
...
![image-20210322171209455](../assets/django/advanced/middleware.assets/image-20210322171209455.png)
## 自定义中间件
...
![image-20210322212937585](../assets/django/advanced/middleware.assets/image-20210322212937585.png)
### 使用类
```
...
```
**查看效果**
添加完中间件后，你访问任何非LOGIN\_URL和OPEN\_URLS里的urls，都需要你先进行登录，如下所示：
![image-20210322222204474](../assets/django/advanced/middleware.assets/image-20210322222204474.png)
## 其它中间件钩子函数
...
Django会在调用视图函数之前调用process\_view方法。它应该返回None或一个HttpResponse对象。 如果返回None，Django将继续处理这个请求，执行任何其他中间件的process\_view方法，然后在执行相应的视图。 如果它返回一个HttpResponse对象，Django不会调用适当的视图函数。 它将执行中间件的process\_response方法并将应用到该HttpResponse并返回结果。
### process\_exception(self, request, exception)
该方法两个参数:
* 一个HttpRequest对象
* 一个exception是视图函数异常产生的Exception对象。
这个方法只有在视图函数中出现异常了才执行，它返回的值可以是一个None也可以是一个HttpResponse对象。如果是HttpResponse对象，Django将调用模板和中间件中的process\_response方法，并返回给浏览器，否则将默认处理异常。如果返回一个None，则交给下一个中间件的process\_exception方法来处理异常。该方法常用于发生异常时通知管理员或将其日志的形式记录下来。
